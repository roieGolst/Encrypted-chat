...
export default class DataHandeler implements IHandler {
    ...
    private async registerLogic(data: RequestObject): Promise<IResponse<string>> {
        ...
        const registerResult = await useCases.register(data.userAttributs);
    ...
    private async loginLogic(data: RequestObject): Promise<IResponse<string>> {
        ...
        const loginResult = await useCases.login(data.userAttributs);
        ...
        const tokens = useCases.authentication.getTokens(user);
    ...
    private createRoom(date: RequestObject): IResponse<string> {
        ...
        const { result, isError } = useCases.authentication.authValidate(date.token);
        ...
        const room = useCases.createRoomChat({
    ...
    private joinChat(data: RequestObject): IResponse<string> {
        ...
        const { result, isError } = useCases.authentication.authValidate(token);
    ...
    private sendNewToken(data: RequestObject): IResponse<string> {
        ...
        const { result } = useCases.authentication.authRefreshToken(refreshToken);
